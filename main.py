...
def test_crc():
    ...
    spa = balboa.BalboaSpaWifi('hottub')
...
async def connect_and_listen(spa_host):
    ...
    for i in range(0, 4):
        ...
        if (msg is None or spa.find_balboa_mtype(msg) != BMTR_MOD_IDENT_RESP):
            msg = await spa.read_one_message()
        if (msg is not None and spa.find_balboa_mtype(msg) == BMTR_MOD_IDENT_RESP):
    ...
    for i in range(0, 4):
        ...
        msg = await spa.read_one_message()
        if (msg is None or spa.find_balboa_mtype(msg) != BMTR_DEVICE_CONFIG_RESP):
            msg = await spa.read_one_message()
        if (msg is not None and spa.find_balboa_mtype(msg) == BMTR_DEVICE_CONFIG_RESP):
            print("Got msg: {0}".format(msg.hex()))
    ...
    for i in range(0, 4):
        ...
        msg = await spa.read_one_message()
        if (msg is None or spa.find_balboa_mtype(msg) != BMTR_SYS_INFO_RESP):
            msg = await spa.read_one_message()
        if (msg is not None and spa.find_balboa_mtype(msg) == BMTR_SYS_INFO_RESP):
            print("Got msg: {0}".format(msg.hex()))
    ...
    for i in range(0, 4):
        ...
        msg = await spa.read_one_message()
        if (msg is None or spa.find_balboa_mtype(msg) != BMTR_SETUP_PARAMS_RESP):
            msg = await spa.read_one_message()
        if (msg is not None and spa.find_balboa_mtype(msg) == BMTR_SETUP_PARAMS_RESP):
            print("Got msg: {0}".format(msg.hex()))
    ...
    for i in range(0, 4):
        ...
        msg = await spa.read_one_message()
        if (msg is None or spa.find_balboa_mtype(msg) != BMTR_FILTER_INFO_RESP):
            msg = await spa.read_one_message()
        if (msg is not None and spa.find_balboa_mtype(msg) == BMTR_FILTER_INFO_RESP):
            print("Got msg: {0}".format(msg.hex()))
    ...
    for i in range(0, 4):
        ...
        msg = await spa.read_one_message()
        if (msg is None or spa.find_balboa_mtype(msg) != BMTR_STATUS_UPDATE):
            msg = await spa.read_one_message()
        if (msg is not None and spa.find_balboa_mtype(msg) == BMTR_STATUS_UPDATE):
            print("Got msg: {0}".format(msg.hex()))
...
async def mini_engine(spahost, temp_diff=2):
    ...
    await temp_play(spa,
                    spa.tmax[spa.temprange][spa.tempscale] if spa.settemp != spa.tmax[spa.temprange][spa.tempscale] else
                    spa.tmax[spa.temprange][spa.tempscale] - 2)
    # await temp_play(spa, save_my_temp)
    await temp_play(spa, save_my_temp + temp_diff)
    print()
...
async def raise_spa_temp(spahost, temp_diff=2):
    """ Test a miniature engine of talking to the spa."""
    spa = balboa.BalboaSpaWifi(spahost)
    await spa.connect()
    ...
    print("Trying to set temperatures")
    print("--------------------------")
    if (spa.settemp + temp_diff) > spa.tmax[spa.temprange][spa.tempscale]:
        print("Spa at max temp: {}  NOT increasing temp.")
    else:
        await temp_play(spa, spa.settemp + temp_diff)
    print()

    await spa.disconnect()


async def lower_spa_temp(spahost, temp_diff=2):
    """ Test a miniature engine of talking to the spa."""
    spa = balboa.BalboaSpaWifi(spahost)
    await spa.connect()
    ...
    print("Trying to set temperatures")
    print("--------------------------")
    if (spa.settemp - temp_diff) < spa.tmin[spa.temprange][spa.tempscale]:
        print("Spa at min temp: {}  NOT lowering temp.")
    else:
        await temp_play(spa, spa.settemp - temp_diff)
    print()
...
async def temp_play(spa, temp):
    ...
    result += f'Current temp is {spa.curtemp}'
    ...
    for i in range(1, 6):
        ...
        if (curSetTemp == temp):
            result += f'  Set Temp is now {curSetTemp}'
            print(result)
            return
        else:
            await asyncio.sleep(i)
    result += f'  Set Temp was not changed after {i} seconds'
    print(result)


async def pump_play(spa, pump, setting):
    print(f'Current Status: {spa.get_pump(pump, text=True)}')
    print(f'  Set to {text_pump[setting]}')
    ...
    for i in range(1, 6):
        ...
        if (curPumpStatus == setting):
            ...
            await asyncio.sleep(i)
    print(
        f'  Pump Status was not changed to {text_pump[setting]} after {i} seconds')
...
async def heatmode_play(spa, to_heatmode):
    ...
    if to_heatmode == spa.HEATMODE_RNR:
        ...
        return
    print(f'  Set to {text_heatmode[to_heatmode]}')
...
if __name__ == "__main__":
    ...
    parser.add_argument('spa', type=str,
                        help='hostname of the spa')
    parser.add_argument("-r", "--raisetemp", type=str, help="Raise Spa Temp")
    parser.add_argument("-l", "--lowertemp", type=str, help="Lower Spa Temp")
    parser.add_argument("-s", "--status", action="store_true", help="show status")

    args = parser.parse_args()

    if args.raisetemp:
        asyncio.run(raise_spa_temp(args.spa, float(args.raisetemp)))
    if args.lowertemp:
        asyncio.run(lower_spa_temp(args.spa, float(args.lowertemp)))
    if args.status:
        print("******* Testing CRC **********")
        test_crc()
        print("******** Testing basic commands **********")
        asyncio.run(connect_and_listen(args.spa))
    print("Result is: " + result)

    slack_webhook_url = environ.get('SLACK_WEBHOOK_URL')
    if slack_webhook_url is not None:
        webhook = WebhookClient(slack_webhook_url)
        response = webhook.send_dict({"username": "Hot Tub Time Machine",
                                      "text": result,
                                      "icon_emoji": ":hottub:",
                                      })
